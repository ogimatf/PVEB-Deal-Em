...
import { isLoged, player1Name } from "./game_state.js";
...
export const initHomeScreen = () => {
  ...
  buttonIgraj.onclick = () => {
    if (!isLoged) {
      alert("Morate biti ulogavani za igranje");
      return;
    }

    waitOpponentWindow();

    socket.emit("enterToLobby", {
      name: player1Name,
    });
  };
...
const waitOpponentWindow = () => {
  let windowMsg = "Trazenje protivnika..";
